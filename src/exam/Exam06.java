...
a. 객체 자신을 가리키는 참조변수이다.  -- 자기주소를 저장하고있다.
b. 클래스 내에서라면 어디서든 사용할 수 있다. → 인스턴스메서드에서만 사용가능 , 
c. 지역변수와 인스턴스변수를 구별할 때 사용한다.
d. 클래스 메서드 내에서는 사용할 수 없다.  --인스턴스에서만 사용가능
...
c. 리턴타입이 달라야 한다.    --상관없다.
d. 매개변수의 이름이 달라야 한다.  -- 상관없음
*
*[6-13] 다음 중 아래의 add메서드를 올바르게 오버로딩 한 것은? (모두 고르시오) b,c,d
  long add(int a, int b) { return a+b;}
a. long add(int x, int y) { return x+y;}      -- 매개변수의 개수가 같아서 안됨, 할려면 없애거나 변수의값을 변경
...
e. 힙(heap)영역에 생성되며 가비지 컬렉터에 의해 소멸된다.   -호출스택에서 생성
*
*
...
*
*
*(1) int[] shuffle(int[] arr) {

static for(int i=0; i< arr.length * 10; i++) {
int random = (int)(Math.random()*arr.length);
// arr[0]와 arr[random]의 값을 서로 바꾼다.
int temp = arr[0];
arr[0] = arr[random];
arr[random] = temp;
}
...
*class Exercise6_22 {
	...
	static boolean is Number(String str){
...
*class Exercise6_24
		...
		int abs(int value){
			return value >= 0 ? value : -value;			주어진 값의 절대값을 반환한다.  
		}
